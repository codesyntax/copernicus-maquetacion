...
require([
...
], function(Map, MapView, ScaleBar, BasemapGallery, Zoom, Measurement, Legend, WMSLayer, GroupLayer, Graphic, Extent, TimeSlider) {
  var map = new Map({
    basemap: "topo-vector",
    slider: false
  });
  ...
  //   url: "https://geoserveis.icgc.cat/icgc_sentinel2/wms/service?service=WMS",
  //   sublayers: [
  //     {              name:"sen2rgb"
  //     }
  //   ]
  ...
  area_button.addEventListener('click', function(evt){
    if (this.classList.contains("esri-icon-cursor-marquee")) {
      ...
      this.classList.add("esri-icon-right-arrow");
      $.each($( ".area-panel input[type=radio]:checked" ), function( i, widget ) {
        $(widget).trigger( "click" );
      })
  ...
  function mapAddLayers(layer) {
    ...
    var time_series = $(layer).find(".map-layer-name").attr("time-series");
    ...
    if (time_series) {
      document.querySelector(".esri-time-slider").style.display = "block"
      view.whenLayerView(layer).then(function (lv) {
        //console.log(layer.timeInfo);
        //*** Add layer fullTimeExtent ***//
        ...
        $.each( component.Products, function( i, product ) {
          ...
          $.each( product.Datasets, function( i, dataset ) {
            ...
            $.each( dataset.Layer, function( i, layer ) {
              var time_series = dataset.IsTimeSeries ? "time-series" : "";
              $("#dataset_"+ component_index +'_'+ product_index +'_'+ dataset_index +" .map-menu-layers-container").append(
                '<div class="ccl-form-group map-menu-layer" id="layer_'+ component_index +'_'+ product_index +'_'+ dataset_index +'_'+ layer_index +'">'+
                  '<input type="hidden" class="map-layer-name" value='+ layer.LayerId +' time-series='+time_series+'>'+
  ...
  var nutsGroupLayer = new GroupLayer({
    title: "nuts",
    opacity: 0.5
  });
  map.add(nutsGroupLayer)

  //*** Download ***//
  $(document).on("click",".area-panel input[type=radio]:checked", function(){
    show_loading = true;
    nutsGroupLayer.removeAll();
    ...
    switch (this.value) {
      case "nuts0":
        ...
        var layer = new WMSLayer({
          ...
          customParameters: {
            layerDefs: "{'0':'LEVL_CODE=0'}",
          },
          infoTemplate: infoTemplate,
        });
        nutsGroupLayer.add(layer);
        // view.when(function () {
